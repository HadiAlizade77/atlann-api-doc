  ...
  <div id="faq-page">
    ...
    <div class="faq-jumbotron">
      <div class="faq-jumbotron-content lg:p-32 md:p-24 sm:p-16 p-8 rounded-lg mb-base">
        <h1 class="mb-1 text-white">Looking for a specific item ?</h1>
        <vs-input placeholder="Search" v-model="faqSearchQuery" icon-pack="feather" icon="icon-search" size="large"
                  class="w-full mt-6" icon-no-border/>
      </div>
    </div>
    <div class="vx-row">
      <div class="vx-col w-full md:w-1/4 lg:w-1/4 rounded-lg">
        <vx-card>
          <h4>Categories List</h4>
          <ul class="faq-topics mt-4">
            <li class="p-2  hover:text-success  font-medium flex items-center">
              <feather-icon icon="CodepenIcon" svgClasses="h-10 w-10 pr-3"/>
              <span class="cursor-pointer">API Overview</span>
            </li>
            <div v-if="categories.length > 0 ">
              <li v-for="(category,index) in categories" :key="index"
                  class="p-2  hover:text-primary  font-medium flex items-center" @click="faqFilter = category.id">
                <feather-icon icon="TerminalIcon" svgClasses="h-10 w-10 pr-3"/>
                <span class="cursor-pointer">{{ category.data.title }}</span>
              </li>
            </div>
          </ul>
        </vx-card>
      ...
      <!-- FAQ COL -->
      <div class="vx-col w-full md:w-3/4 lg:w-3/4 mt-12 md:mt-0">
        <div class="vx-row">
          <div class="vx-col w-full md:w-3/8">
              <vx-card
          title="Content Color"
          title-color="primary"
          class="px-2"
          ...
          code-toggler>
          <p class="mb-3">You can use <code>content-color</code> prop to change color of title of card. This prop
            supports hex, rgba, rgb and theme colors.</p>
        </vx-card>
          </div>
        </div>
        \
...
export default {
  ...
  computed: {
    ...
    categories() {
      return this.$store.getters['doc/getCategories']
    }
  },
  created() {
